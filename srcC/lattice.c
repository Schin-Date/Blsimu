...
//unused double x1, xp1, y1, yp1, x2, xp2, y2, yp2, x, xp, y, yp, z;
double x1, xp1, y1, yp1, x2, xp2, y2, yp2;
double el, pel, px,py,pz,pt2,pp2;
double s0, s1, z1, s2, z2, l, lenB, lenarc, rK;
double thetb0, tanb0, cosb0, rho0, rho;
//unused double xB1, xpB1, CB1, SB1, xB2, xpB2, CB2, SB2;
double xB1, xpB1, xB2, xpB2;
double Uh[4], Uv[4];
double A[4],T[4],F[4],D[4];
//unused int ipcl, ielm, i,j,k;
int ipcl, ielm;
int ifin,  ntrap, nfin;
...
//    printf("...track_lattice: R4OVER/UNDR = %e/%e\n",R4OVER,R4UNDR);//<<<<<<<<<<<<<<<<<<<<
...
/*===========================================================================*/
int read_lattice(FILE *logstr, char const *infile, double smin, double smax){
FILE *instr;
...
/*061115     */ 
      ...
      nline = 5000; scolumn = 1; icolumn = 3; rcolumn = 16;
           Idata = (int *) calloc(nline*icolumn,sizeof(int));
           Rdata = (double *) calloc(nline*rcolumn,sizeof(double));
           Sdata = (char **) calloc(nline*scolumn,sizeof(char*));
           for(i=0;i<nline*scolumn;i++){
              *(Sdata+i) = (char *) calloc(64,sizeof(char));}
