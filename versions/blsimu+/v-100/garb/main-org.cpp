...
main(int argn,char* argv[]) {
    ...
    string doprmfilename = "blsimu+main.doprm";//the default doprmfile
...
//    METER = plnck::meter;
    ...
    witgn::SetDoPrm Useinp("blsimu.doprm");
//    witgn::SetDoPrm Useinp("Useinp.doprm");
//    int PrtLvl = Useinp.GetPrtLvl();
//    Useinp.SetPrtLvl(3);
//    Useinp.SetPrtLvl(PrtLvl);

    /* prepare for an output file */
//    std::ofstream ofs("main.out");
    std::ofstream ofs(Useinp.getValue<std::string>("outfile").c_str());
    ...
    /* induced parameters */
    double Llambda = Useinp.getValue<double>("Llambda")*plnck::nm;
...
//    double Elaser = (WL2E / Llambda);  /* Laser energy */
    ...
    //optfile
    string optfile = Useinp.getValue<string>("optfile");
    double smin = Useinp.getValue<double>("smin")*plnck::meter;
    double smax = Useinp.getValue<double>("smax")*plnck::meter;
//    double R4OVER = mainprm.getValue<double>("R4OVER");
    std::cout<<"...main::going to read "<<optfile
    <<"\n                  << (smin, smax) = ("
    <<smin/plnck::meter<<", "<<smax/plnck::meter<<") [m]"<<std::endl;
    //******** calling a c function optics.c::read_opt()*******/
    FILE *logstr = fopen(Useinp.getValue<std::string>("logfile").c_str(),"w");
        int status = read_opt(logstr,optfile.c_str(),smin/plnck::meter,smax/plnck::meter);
        //======== this is a c function =========================
    //latfile
    string latfile = Useinp.getValue<string>("latfile");
...
#endif
    double Lwaist = Useinp.getValue<double>("Lwaist")*plnck::mm;
    double Lfocus = Useinp.getValue<double>("Lfocus")*plnck::meter;
    double Lsstart = Useinp.getValue<double>("Lsstart")*plnck::meter;
    double Lsend = Useinp.getValue<double>("Lsend")*plnck::meter;
...
#endif
    std::string tagger = Useinp.getValue<string>("tagger");
    ...
    int nbmpcl = Useinp.getValue<int>("nbmpcl");
...
//    PSphot = calloc(nbmpcl*4,sizeof(double));
    ...
    int randseed = Useinp.getValue<int>("randseed");
    setrandom(randseed);
    ...
    //Laser profile
    std::string Lprof = Useinp.getValue<string>("Lprof");
    //electron beam phase space distribution
    std::string cntl_ebeam = Useinp.getValue<string>("cntl_ebeam");
    //event control
    std::string cntl_event = Useinp.getValue<string>("cntl_event");
    //position of the last element of lattice
    double slastelm = Useinp.getValue<double>("slastelm")*plnck::meter;
    //Tagging parameters
    double Tags1 = Useinp.getValue<double>("Tags1")*plnck::meter;
    double Tags2 = Useinp.getValue<double>("Tags2")*plnck::meter;
    double BRz1 = Useinp.getValue<double>("BRz1")*plnck::meter;
    double BRz2 = Useinp.getValue<double>("BRz2")*plnck::meter;
    double Tagx0 = Useinp.getValue<double>("Tagx0")*plnck::meter;
    double Tagxd = Useinp.getValue<double>("Tagxd")*plnck::meter;
    //e-beam parameters
    double pel0 = Useinp.getValue<double>("pel0")*plnck::MeV;
    double e0spread = Useinp.getValue<double>("e0spread");
    double emittance_x = Useinp.getValue<double>("emittance_x")
                                *plnck::meter*plnck::radian;
    double emittance_y = Useinp.getValue<double>("emittance_y")
                                *plnck::meter*plnck::radian;
    //RF parameter
    double OVRVLR = Useinp.getValue<double>("OVRVLR");
    //chamber physical apertures
    double CHMBDX = Useinp.getValue<double>("CHMBDX")*plnck::meter;
    double CHMBDY = Useinp.getValue<double>("CHMBDY")*plnck::meter;
    //
    
    //<<<<<<<<<<<<<<<<<<<<<< Event Loop <<<<<<<<<<<<<<<<<<<<<<
    int Nevent = Useinp.getValue<double>("Nevent");
    int ievent = 0;
    int nrejec = 0;
