...
//    PSphot = calloc(nbmpcl*4,sizeof(double));
    ...
    while(ievent<Nevent){  ++ievent;
        ...
        //if(strstr(cntl_event,"EgCut_ON")!=0){
//        int ioffset = cntl_event.find("EgCut_ON");
//        if(ioffset!=std::string::npos){
        if(cntl_event.find("EgCut_ON")!=std::string::npos){
            /*         -------------------------------    */
            /*#ifdef CutEg */
            if(*(PSphot) > 2400. || *(PSphot) < 2200. ){ //<<< manually given
                ++nrejec;
                --ievent;
                continue;
            }
