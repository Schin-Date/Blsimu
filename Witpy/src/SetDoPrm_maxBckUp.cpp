...
//SetDoPrm::SetDoPrm(const char* doprmfilename)
SetDoPrm::SetDoPrm(const std::string doprmfilename)
{
	
	if (5<s_PrtLvl) std::cout << "D6...SetDoPrm(\""<<doprmfilename
    ...
    *pClass, *pInstance, *pValue;
    ...
    //NG in Py3 : pName = PyString_FromString("readDoPrm");
    //pName = PyBytes_FromString("readDoPrm");
    std::cout<<"D...main::about to call PyBytes_FromString..."<<std::endl;
    pName = PyUnicode_DecodeFSDefault("readDoPrm");
    
    // Load the module object
    std::cout<<"D...main::about to call PyImport_Import..."<<std::endl;
    pModule = PyImport_Import(pName);
    
    //pDict is a borrowed reference
    std::cout<<"D...main::about to call PyModule_GetDict..."<<std::endl;
    pDict = PyModule_GetDict(pModule);
    
    //pFunc is also a borrowed reference
    //std::cout<<"D...main::about to call PyDict_GetItemString..."<<std::endl;
    //F pFunc = PyDict_GetItemString(pDict, "multiply");
    ...
    if (PyCallable_Check(pClass)) {
        ...
        std::cout<<"D...main::about to call PyObject_CallObject..."<<std::endl;
        pInstance = PyObject_CallObject(pClass, NULL); //are vars in init required to pass?
    }
    else {
        PyErr_Print();
    }
    ...
    //
    std::cout<<"D...main::about to call PyObject_CallMethod..."<<std::endl;
    ...
    std::cout<<"...pValue: List size = "<<lsize<<std::endl;
...
#if(0) //0
    // Read doprm file and call another constructor to read an asc data file
    //	std::string dolisttmpl="type Variable value comment";//Provide a template line of the doprm data file
    std::string dolisttmpl="type Variable value";//Provide a template line of the doprm data file
    
	this->clear_table();
	this->put_line(dolisttmpl);	
	//get datalinestruct
	std::string datalinestruct = this->get_line_format(dolisttmpl);
	if (8<s_PrtLvl) std::cout << "D9"
		<<"...SetDoPrm: get_line_format() = '"
		<< datalinestruct
		<<"' "<<datalinestruct.length()<<" columns"  <<std::endl;
	
	//Open the prm file 
	std::ifstream ifs(doprmfilename.c_str());
	if (ifs.fail()) {
		std::cerr <<"E...SetDoPrm:"
		<<" doprmfilename = '"<<doprmfilename<<"': File open failuer."<<std::endl;
		exit(1);
	}	
	
	int nline=-1; // storage to get the number of data lines
	int nskip = 0; // number of header lines
	
	if (0<s_PrtLvl) std::cout << "N1"
		<< "...SetDoPrm: about to call eatTable < "
		<<doprmfilename<<std::endl;
	
	this->eatTable(ifs, nskip, datalinestruct, nline,"end_by_enddoprm allow_comment_section");
...
//	int icountV=0;
	...
	for (int iline=0; iline<nline; iline++) {
		...
		std::string variable_type = this->get_table(iline ,0);
		std::string variable_name = this->get_table(iline ,1);
		std::string variable_value = this->get_table(iline ,2);
//		std::string variable_comment = this->get_table(iline ,3);
		
		if ('#'==variable_type.at(0)) {
			continue;			
		}
		if ("enddoprm"==variable_type) {
			break;			
		}
...
//		m_map_prm.insert(make_pair(variable_name,variable_value));
		...
		if (2 < s_PrtLvl&&("PrtLvl"!=variable_name)) std::cout <<"N3"
			<<"...SetDoPrm: i="<<std::setw(2)<< iline
			<<": "<<variable_type
			<<" \""<<variable_name<<"\""
	...
	if(true == this->is_a_key("PrtLvl")) {
		s_PrtLvl = this->getValue<int>("PrtLvl");
		if (4<s_PrtLvl) std::cout << "N5"
			<<"...SetDoPrm::SetDoPrm("<<doprmfilename<<"): "
			<<"s_PrtLvl = "<<s_PrtLvl<<std::endl;
	}

	if (0<s_PrtLvl) std::cout << "N1"
			<<"...SetDoPrm::SetDoPrm("<<doprmfilename<<"): "
			<<m_nmaps<<" tags are mapped."<<std::endl;
#endif //0
}
