...
def set_printLvl(val):
    global printLvl_
    printLvl_ = val
    if 5 < printLvl_:
        print('d6...readDoPrm.set_printLvl: printLvl_ = ',printLvl_)
    return printLvl_
...
#1        #if 1< printLvl: print('readData::init')
    def __init__(self):
        if 5<printLvl_: print('d6...ReadDoPrm:: __init__: called...')
    #-----------------------------
...
#=================================================================================
    def txt2dataLst(self,in_txtFile='',out_logFile=''):
        ...
        if 6<printLvl_: print('d7...ReadDoPrm::txt2dataLst: printLvl_({}) out_logFile({}) called...'.
                              format(printLvl_, out_logFile))
        ...
        if 7<printLvl_: print('D8...setting fLog...')
        #if ''==out_logFile: fLog = sys.stdout
        #else:
        fLog = None
        if 'NOLOG'!=out_logFile: fLog = open(out_logFile,'w')
        
        if fLog: fLog.write('date: '+str(datetime.today())+'\n')
...
#-------------------------------------------------------------- Parsing
            ...
            try:
                ...
                if 'string' == vartype_s or 'str' == vartype_s or '' == vartype_s:
                    vartype_s = 'string'
                    #valD = '"'+rightValue_s+'"'
                    #valD = str(rightValue_s)
                    valD = rightValue_s
                elif 'bool' == vartype_s or 'Bool_t' == vartype_s:
                    vartype_s = 'bool'
                    valD = rightValue_s.startswith('t')
                else:
                    print('E...readDoPrm::txt2data: varName="{}"; rightValue_s : {}\n'.
                         format(varName,rightValue_s))
                    break
            ...
            logline_lst =['      macro_l {} : "{}"\n'.format(iline, macro_l)]
            self._printLog(logline_lst,fLog)
            if 6<printLvl_: print('D7...macro_l {} : "{}"'.
                                  format(iline,macro_l))
            exec(macro_l)
            #+++++++++++++++++
            ...
            self.data_lst.append([vartype_tag, varName, py_val])
    ...
    '''
    print('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<here: varName : {}, vartype_s : {}, valD : {}'.
    format(varName,vartype_s, valD))
    '''
